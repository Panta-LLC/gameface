...
const RedisExport = RedisPubSubModule.RedisPubSub ||
    (RedisPubSubModule.default && RedisPubSubModule.default.RedisPubSub) ||
    RedisPubSubModule.default ||
    RedisPubSubModule;
...
const redisPubSub = new RedisExport();
export function createSignalingServer(httpServer) {
    ...
    const broadcastToRoom = (room, payload, exclude) => {
        const set = rooms.get(room);
        if (!set)
            return;
    ...
    const gameSelection = new Map();
    const activitySelection = new Map();
    const readiness = new Map();
    const tables = new Map();
    const initEmptyTable = (gameId, players = 4, hostId) => {
        const seats = Array.from({ length: players }).map((_, i) => ({
            index: i,
            playerId: null,
        }));
        return { gameId, seats, status: 'lobby', hostId };
    };
    const setGameForRoom = (room, game) => {
    ...
    const persistRoomState = async (room) => {
        try {
            ...
            if (typeof redisPubSub.setWithTTL === 'function') {
                await redisPubSub.setWithTTL(`signaling:room:${room}:state`, payload, ttl);
            }
            else {
    ...
    const startGame = (room) => {
        ...
        if (!game)
            return console.warn('start game without game selection');
    ...
    void redisPubSub
        ...
        void redisPubSub.psubscribe('signaling:room:*', (msg, _channel) => {
            try {
                if (!msg || msg.source === instanceId)
        ...
        ws.on('message', async (message) => {
            ...
            switch (data.type) {
                case 'join': {
                    ...
                    try {
                        ...
                        if (persisted) {
                            ...
                            if (persisted.game)
                                gameSelection.set(room, persisted.game);
                            if (persisted.activity)
                                activitySelection.set(room, persisted.activity);
                            if (persisted.tableState)
                                tables.set(room, persisted.tableState);
                        }
                    }
                    catch (e) {
                ...
                case 'select-game': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('select-game without room');
                    if (!data.game)
                        return console.warn('select-game missing game');
                ...
                case 'select-activity': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('select-activity without room');
                    if (!data.activity)
                        return console.warn('select-activity missing activity');
                ...
                case 'offer': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('offer without room');
                ...
                case 'answer': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('answer without room');
                ...
                case 'candidate': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('candidate without room');
                ...
                case 'start-game': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('start-game without room');
                    startGame(room);
                    break;
                }
                ...
                case 'cardtable.seat.claim': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('cardtable.seat.claim without room');
                ...
                case 'cardtable.seat.release': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('cardtable.seat.release without room');
                    const { seatIndex, playerId, tableId } = data;
                    if (typeof seatIndex !== 'number' || !playerId)
                ...
                case 'cardtable.seat.update': {
                    ...
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('cardtable.seat.update without room');
                    const { tableState } = data;
                    if (!tableState)
                        return console.warn('cardtable.seat.update missing tableState');
                    // Basic validation/normalization
                    const normalized = {
                        gameId: tableState.gameId ?? null,
                        seats: Array.isArray(tableState.seats)
                            ? tableState.seats.map((s, i) => ({
                                index: i,
                ...
                case 'cardtable.start': {
                    const room = socketRoom.get(ws);
                    if (!room)
                        return console.warn('cardtable.start without room');
                    const { tableState } = data;
                    const cur = tables.get(room) || (tableState ? tableState : initEmptyTable(null, 4));
                    // validate seats full
                    const filled = cur.seats.every((s) => s.playerId !== null);
                    if (!filled) {
