...
> **Status: deferred.** The ECS Fargate deployment path this guide configures
> is parked under [`terraform_deferred/`](../terraform_deferred/), and
> `deploy.yml` is now a `workflow_dispatch`-only guard stub. The active
> deployment is single-host EC2 + Docker Compose — see
> [`DEPLOYMENT.md`](./DEPLOYMENT.md). Revive this guide when ECS work resumes
> (criteria in [`terraform_deferred/README.md`](../terraform_deferred/README.md)).
