# GameFace Technical Prompts – Index

Use this index to navigate all Epic implementation prompts. Start with an Epic’s README to understand scope and acceptance criteria, then use the linked task prompts to implement features consistently.

## How to use these prompts
See the step-by-step guide in [USAGE.md](./USAGE.md).

---
## Epic 1 – User Authentication and Session Management
Folder: [epic1](./epic1/README.md)
- Registration: user-registration-implementation-prompt.md
- Login: user-login-implementation-prompt.md
- Session persistence: user-session-persistence-implementation-prompt.md
- Logout: user-logout-implementation-prompt.md

## Epic 2 – Real-Time Video Communication
Folder: [epic2](./epic2/README.md)
- p2p-video-implementation-prompt.md
- audio-video-controls-implementation-prompt.md
- connection-stability-implementation-prompt.md
- signaling-implementation-prompt.md

## Epic 3 – Mini-Game Integration
Folder: [epic3](./epic3/README.md)
- game-selection-implementation-prompt.md
- game-sync-implementation-prompt.md
- basic-games-implementation-prompt.md

## Epic 4 – Scalable Backend Architecture
Folder: [epic4](./epic4/README.md)
- backend-apis-implementation-prompt.md
- redis-pubsub-implementation-prompt.md
- mongodb-implementation-prompt.md

## Epic 5 – Frontend Development
Folder: [epic5](./epic5/README.md)
- ui-system-implementation-prompt.md
- responsive-design-implementation-prompt.md
- error-handling-notifications-implementation-prompt.md

## Epic 6 – Performance Optimization
Folder: [epic6](./epic6/README.md)
- load-testing-implementation-prompt.md
- caching-implementation-prompt.md
- webrtc-performance-tuning-implementation-prompt.md

## Epic 7 – Security & Privacy
Folder: [epic7](./epic7/README.md)
- encryption-implementation-prompt.md
- credential-secrets-implementation-prompt.md
- security-audit-privacy-implementation-prompt.md

## Epic 8 – Deployment & Monitoring
Folder: [epic8](./epic8/README.md)
- ci-cd-implementation-prompt.md
- containerization-deployment-implementation-prompt.md
- observability-implementation-prompt.md

## Epic 9 – Testing & Quality Assurance
Folder: [epic9](./epic9/README.md)
- testing-strategy-implementation-prompt.md
- load-testing-qa-implementation-prompt.md
- ci-test-automation-implementation-prompt.md

---
## Conventions
- Each prompt includes: Context, Objectives, Acceptance Criteria, and a Test Plan.
- Keep commit messages scoped to the prompt’s focus (feat/fix/chore), reference Epic/Story if applicable.
- Update dashboards, runbooks, and docs as specified in prompts to keep operational readiness.
